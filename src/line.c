...
struct line {
	...
	uint32_t* chars; // An array of characters contained in this line, excluding the trailing \n
...
void line_realloc(struct line* l) {
	if (l->len + 1 > l->cap) {
		...
		l->chars = (uint32_t*) realloc(l->chars, l->cap * sizeof(uint32_t));
	}
}
...
int line_read_from(struct line* l, int at, FILE* s) {
	...
	while (1) {
		...
		if (at < l->len) {
			// TODO: optimize this
			memmove(&l->chars[at+1], &l->chars[at], (l->len-at)*sizeof(uint32_t));
		}
...
// if necessary.
void line_insert_char(struct line* l, int at, uint32_t c) {
	if (at > l->len) {
		at = l->len;
	}
...
void line_delete_range(struct line* l, int at, int len) {
	...
	memmove(&l->chars[at], &l->chars[at+len], (l->len-at-len)*sizeof(uint32_t));
	l->len -= len;
...
// value.
uint32_t line_delete_char(struct line* l, int at) {
	if (l->len == 0) {
		return -1;
	}
	if (at < 0) {
...
struct line* line_split(struct line* l, int at) {
	...
	if (at < l->len) {
		...
		if (at == 0) {
			...
			next->len = l->len - at;
			next->chars = (uint32_t*) malloc(next->len * sizeof(uint32_t));
			memcpy(next->chars, &l->chars[at], next->len * sizeof(uint32_t));
			l->len = at;
		}
	}
...
void line_join(struct line* l, struct line* other) {
	...
	if (other->len > 0) {
		...
		memcpy(&l->chars[l->len], other->chars, other->len * sizeof(uint32_t));
	}
	l->len = len;
}
